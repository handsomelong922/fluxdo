...
class _AiChatPageState extends ConsumerState<AiChatPage> {
  ...
  ({AiProvider provider, AiModel model})? _firstModelForMode(PromptType mode) {
    ...
    final defaultModel = ref.read(defaultAiModelProvider);
    if (defaultModel != null && matches(defaultModel.model)) {
      return defaultModel;
    }
  ...
  Future<void> _sendPresetPrompt(String prompt, {String? aspect}) async {
  ...
  Future<void> _sendConfiguredQuickPrompt(
    String prompt, {
  ...
  Widget _buildConfiguredTextQuickActions(ThemeData theme) {
        ...
        Wrap(
          ...
          children: quickPrompts.map((item) {
            return ActionChip(
              ...
              onPressed: () {
                unawaited(
                  _sendConfiguredQuickPrompt(
                    item.prompt,
                    scopeOverride: item.scope,
                  ),
                );
              },
            );
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      if (!isImage) ...[
                        ...
                        _buildConfiguredTextQuickActions(theme),
                      ],
                    ],
                  ),
                ),
