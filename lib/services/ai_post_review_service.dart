...
class AiPostReviewService {
  ...
  static const guidelinesCacheKey = 'ai_post_review_guidelines_cache';
  static const guidelinesCacheUpdatedAtKey =
  ...
  Future<AiPostReviewResult> review(AiPostReviewRequest request) async {
    ...
    try {
      await for (final token in _chatService.sendChatStream(
  ...
  Future<_GuidelinesLoadResult> _loadGuidelines() async {
    try {
      ...
      await _prefs.setString(guidelinesCacheKey, text);
      await _prefs.setInt(
        guidelinesCacheUpdatedAtKey,
        DateTime.now().millisecondsSinceEpoch,
      );
      return _GuidelinesLoadResult(text: text, usedCache: false);
    } catch (error, stackTrace) {
      final cached = _prefs.getString(guidelinesCacheKey);
