...
class BrowserTrustCoordinator {
  ...
  Future<bool> _awaitClearanceResolved(
    ...
    void listener() {
      final value = cf.clearanceResolvedAt.value;
      if (value != null && value.isAfter(after) && !completer.isCompleted) {
  ...
  Future<bool> _hydratePreloadThroughWebView({required String reason}) async {
    ...
    try {
      ...
      if (tToken != null && tToken.isNotEmpty) {
        ...
        if (bootstrap.ok && hasRuntimeCookie) {
  ...
  Future<bool> _isNativePreloadTrusted() async {
    ...
    final expiresAt = clearance.expiresAt?.toUtc();
    if (expiresAt == null) {
      _log('native trust check: trusted, no expires');
      return true;
    }
    final ttl = expiresAt.difference(DateTime.now().toUtc());
  ...
  void _log(String message, {String level = 'info'}) {
    CfChallengeLogger.log('[BrowserTrust][$level] $message');
  }
