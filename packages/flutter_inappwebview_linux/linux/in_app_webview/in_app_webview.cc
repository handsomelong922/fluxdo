...
std::optional<std::string> InAppWebView::getMetaThemeColor() const {
...
#if defined(WEBKIT_CHECK_VERSION) && WEBKIT_CHECK_VERSION(2, 49, 3)
  WebKitColor color;
  ...
  return std::string(hexColor);
#else
  // The public theme-color API was added after the WebKit version shipped by
  // the GNOME 48 Flatpak runtime. Older runtimes simply report no theme color.
  return std::nullopt;
#endif
}
